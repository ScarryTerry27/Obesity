...
FIELD_DEFS = [
    ...
    ("qnox", "qNOX", "", "float"),
    ("emg", "EMG", "", "float"),
    ("pn_arterial", "Pн артер.", "мм рт.ст.", "float"),
...
def show_t3_slice():
    ...
    st.title(f"t3 показатели пациента {person.fio}")
    st.caption("после индукции анестезии и интубации трахеи")

    existing = t3_get_result(person.id)
    defaults = existing.model_dump() if existing else {}

    values, submitted = render_slice_form(FIELD_DEFS, defaults, "t3_form")
    if submitted:
