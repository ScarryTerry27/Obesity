...
def show_t8_slice():
    ...
    st.title(f"t8 показатели пациента {person.fio}")
    st.caption("сразу после экстубации трахеи")

    existing = t8_get_result(person.id)
    defaults = existing.model_dump() if existing else {}

    values, submitted = render_slice_form(FIELD_DEFS, defaults, "t8_form")
    if submitted:
